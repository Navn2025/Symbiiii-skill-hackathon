...
router.get('/problems', verifyAuthOptional, (req, res) =>
{
...
router.get('/problems/:id', verifyAuthOptional, (req, res) =>
{
    const {id}=req.params;
    let problem=null;
...
router.post('/problems/:id/run', verifyAuth, async (req, res) =>
{
    const {id}=req.params;
    ...
    try
    {
        ...
        // TODO: Replace with actual code execution via sandbox (Docker, Judge0, etc.)
        // For now, using mock results - NOT FOR PRODUCTION
        const isDemoMode=process.env.DEMO_MODE!=='false';

        if (!isDemoMode)
        {
            console.error('[CODE-EXECUTION] Code execution sandbox not configured!');
            return res.status(503).json({
                success: false,
                error: 'Code execution service not available. Please configure sandbox.',
            });
        }

        console.warn('[CODE-EXECUTION] Using DEMO/MOCK mode - results are not real test executions!');
        ...
        res.json({
            ...
            isDemoMode: true,
            warning: 'Demo mode: Results are mocked and not real test executions'
        });
    } catch (error)
    {
...
// ── POST /problems/:id/submit – run code against ALL test cases (auth required) ──
router.post('/problems/:id/submit', verifyAuth, async (req, res) =>
{
    const {id}=req.params;
    const {code, language}=req.body;
    ...
    try
    {
        ...
        console.warn('[CODE-EXECUTION] Using DEMO/MOCK mode for submit - results are not real!');
...
router.post('/problems/:id/hint', verifyAuth, async (req, res) =>
{
    const {id}=req.params;
    const {code, language}=req.body;
