...
function CandidateProfile() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.put(`${API_URL}/api/profile/${userId}`, profile);
      setProfile(res.data.profile);
  ...
  const handleResumeUpload = async () => {
    ...
    try {
      const userId = user.id || user._id;
      const res = await axios.post(`${API_URL}/api/profile/${userId}/resume`, { resumeText });
      setProfile(res.data.profile);
  ...
  const handleFileUpload = async (e) => {
    ...
    if (isPdf) {
      ...
      try {
        ...
        const res = await axios.post(`${API_URL}/api/profile/${userId}/resume/file`, formData, {
  ...
  const handleDeleteResume = async () => {
    try {
      const userId = user.id || user._id;
      await axios.delete(`${API_URL}/api/profile/${userId}/resume`);
      setResumeText('');
      fetchProfile(userId);
  ...
  const handleATSCheck = async (withJd = false) => {
    ...
    try {
      const userId = user.id || user._id;
      const res = await axios.post(`${API_URL}/api/profile/${userId}/resume-analyze`, { jdText: withJd ? jdText : '' });
